...
public class DonationRegisterUpdateTask implements SetupTask {
  ...
  @Override
  public void install() {
    if (!templateDao.templateExists(DonationRegisterConstants.MANUAL_DONATION_NOTICE)) {
    ...
    if (!templateDao.templateExists(DonationRegisterConstants.INCOMING_DONATION_NOTICE)) {
      Template template = new Template();
