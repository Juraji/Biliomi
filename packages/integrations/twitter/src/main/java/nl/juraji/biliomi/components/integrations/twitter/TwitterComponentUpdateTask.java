...
public class TwitterComponentUpdateTask implements SetupTask {
    ...
    @Override
    public void install() {
        if (templateDao.templateMissing(TwitterComponent.TWITTER_UPDATE_TEMPLATE_ID)) {
        ...
        if (templateDao.templateMissing(TwitterComponent.TWITTER_TWEET_FOUND_TEMPLATE_ID)) {
            Template template = new Template();
