...
public class BitsComponentSetupTask implements SetupTask {
  ...
  @Override
  public void install() {
    if (!templateDao.templateExists(BITS_CHEERED_TEMPLATE_ID)) {
      Template template = new Template();
      template.setTemplateKey(BITS_CHEERED_TEMPLATE_ID);
    ...
    if (!templateDao.templateExists(BITS_PAYOUT_TO_CHEERER_TEMPLATE_ID)) {
      Template template = new Template();
      template.setTemplateKey(BITS_PAYOUT_TO_CHEERER_TEMPLATE_ID);
