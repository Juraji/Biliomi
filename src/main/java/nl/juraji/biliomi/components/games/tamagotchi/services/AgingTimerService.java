...
import static nl.juraji.biliomi.components.games.tamagotchi.services.TamagotchiConstants.AGING_INTERVAL_HOURS;
import static nl.juraji.biliomi.components.games.tamagotchi.services.TamagotchiConstants.PROPERTY_DECAY_PER_HOUR;
...
public class AgingTimerService extends TimerService {
  ...
  public void start() {
    super.start();
    scheduleAtFixedRate(this::runAging, AGING_INTERVAL_HOURS, TimeUnit.HOURS);
  }
  ...
  public void processPropDecay(Tamagotchi tamagotchi) {
    ...
    double toyFoodMod = tamagotchiToys.stream().mapToDouble(toy -> PROPERTY_DECAY_PER_HOUR * toy.getFoodModifier()).sum();
    double toyMoodMod = tamagotchiToys.stream().mapToDouble(toy -> PROPERTY_DECAY_PER_HOUR * toy.getMoodModifier()).sum();
    double toyHygieneMod = tamagotchiToys.stream().mapToDouble(toy -> PROPERTY_DECAY_PER_HOUR * toy.getHygieneModifier()).sum();
  ...
  private double getDecayedValue(double value, double propModifier) {
    value -= (PROPERTY_DECAY_PER_HOUR - propModifier);
